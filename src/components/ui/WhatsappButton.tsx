...
export default function WhatsAppButton() {
  ...
  useEffect(() => {
    ...
    // Tampilkan langsung jika sudah di posisi scroll
    handleScroll();
    return () => window.removeEventListener("scroll", handleScroll);
  }, []);

  // Sembunyikan label setelah 4 detik
  useEffect(() => {
    const timer = setTimeout(() => setShowLabel(false), 4000);
    return () => clearTimeout(timer);
  }, []);

  const waUrl = `https://wa.me/${dealerInfo.whatsapp}?text=${encodeURIComponent(
    "Halo, saya ingin bertanya tentang mobil Wuling"
  )}`;
  ...
  return (
    ...
    >
      {/* Label tooltip */}
      {showLabel && (
        <div className="bg-white text-wuling-black text-sm font-medium px-4 py-2 rounded-full shadow-lg border border-gray-100 whitespace-nowrap animate-pulse">
          Chat dengan kami! 👋
        </div>
      )}
      ...
      <a
        ...
        className="w-14 h-14 bg-green-500 hover:bg-green-600 text-white rounded-full shadow-lg hover:shadow-xl flex items-center justify-center transition-all duration-200 hover:scale-110"
      >
        {/* Pulse ring */}
